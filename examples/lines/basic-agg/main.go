...
func main() {
	...
	// Create AGG renderer using the backend registry
	config := backends.SimpleConfig(640, 360, render.Color{R: 1, G: 1, B: 1, A: 1})
	r, err := backends.Create(backends.AGG, config)
	if err != nil {
		fmt.Printf("Error creating AGG renderer: %v\n", err)
		return
	}

	// Save as PNG
	err = core.SavePNG(fig, r, "out-agg.png")
	if err != nil {
		fmt.Printf("Error saving PNG: %v\n", err)
		return
	}

	fmt.Println("Successfully created out-agg.png with AGG renderer!")
}