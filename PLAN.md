...
- `TestAggTextSingleBaselineDiagnostic` now compares the current Go `x/image/opentype` raster path against Matplotlib `force_autohint` (the reference default), Matplotlib `no_hinting`, and a native FreeType diagnostic path using `FT_LOAD_FORCE_AUTOHINT`. Fixed-baseline `"Basic Bars"` still matches Matplotlib `no_hinting` best (`MeanAbs≈0.024`, identical dark-pixel bounds), while the native force-autohint path is worse (`MeanAbs≈0.928`, best shifted `MeanAbs≈0.349`) because it does not yet reproduce Matplotlib ft2font's exact glyph placement/coverage pipeline. The diagnostic also logs Matplotlib's FreeType version (`2.12.1` in the current conda environment) and the cgo-linked native FreeType version (`2.13.2`); this version/library mismatch is a likely source of one-pixel hinted glyph coverage differences. Fixed-baseline `"0"` is closest in the current path versus Matplotlib `force_autohint` (`MeanAbs≈0.056`), with only the bottom edge 1px too low. Keep the native path diagnostic-only until it can be linked against the same FreeType/libraqm stack or reproduce Matplotlib's `_layout`/`_render_glyph` pipeline closely enough; separately preserve the tick-label baseline rounding investigation.
