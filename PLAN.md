...
- [~] Add SVG support for hatches, alpha groups, raster image embedding, transformed images, and Gouraud/gradient fallbacks where vector-native output is practical. *(Transformed images landed: `ImageTransformed` emits `transform="matrix(a b c d e f)"` and the SVG backend now advertises `ImageTransform` as a native capability with a runtime verification test. Hatches, alpha groups, and Gouraud fallbacks still pending.)*
